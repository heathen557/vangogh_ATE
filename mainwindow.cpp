...
void MainWindow::AckCmd_MainWindow_slot(QString returnCmd,QString AckInfo)
{
    ...
    }else if("8182" == returnCmd)                     //下载标定固件后
    {
        ...
        }else                                        //OTP返回的寄存器值 需保存
        {
            ...
            QStringList strList;
            strList<<QStringLiteral("OTP标定结果")<<AckInfo;
            writeTocsv(saveFilelPath,strList);

            //保存至本地
            QString strMsg = QStringLiteral("标定已经完成完成，下一步发送下载测距固件的命令！");
            Disploy_log(strMsg);

            QString cmdStr = "5A 01 00 00 83 01";
            emit sendSerialSignal(cmdStr);
            m_speech->say(strMsg);
    ...
    }else if("8183" == returnCmd)
    {
        ...
        if(1 == flagInt)                             //成功
        {
            QString strMsg = QStringLiteral("下载测距固件完成！下一步发送移动至10cm的命令！");
            Disploy_log(strMsg);

            QString cmdStr = "5A 01 00 00 70 05";
            emit sendSerialSignal(cmdStr);
            m_speech->say(strMsg);
    ...
    }else if("8185" == returnCmd)
    {
        ...
        QString strMsg = QStringLiteral("序列号已经写入完成，下一步发送300mm命令!");
        Disploy_log(strMsg);
        m_speech->say(strMsg);
...
void MainWindow::toSendStatistic_slot(int distance, vector<double> StatisticLSB_vector, vector<double> StatisticMM_vector, vector<double> StatisticPeak_vector, vector<double> detectionRate_vector)
{
    ...
    int len;
    if(p1 == distance)   //发送移动导轨的信号,p3:500mm ;    计算相关的统计信息
    {

        QString cmdStr = "5A 01 00 00 70 07 ";      // 下一步发送 移动至 P2 的距离
        emit sendSerialSignal(cmdStr);
        ...
        QStringList strList;
        strList<<(QString::number(p1)+QStringLiteral("mm均值"))<<QString::number(LSB_mean);
        writeTocsv(saveFilelPath,strList);
        strList.clear();
        strList<<(QString::number(p1)+QStringLiteral("mm方差"))<<QString::number(LSB_std);
        writeTocsv(saveFilelPath,strList);
        strList.clear();
        strList<<(QString::number(p1)+QStringLiteral("mm修正均值"))<<QString::number(MM_mean);
        writeTocsv(saveFilelPath,strList);
        strList.clear();
        strList<<(QString::number(p1)+QStringLiteral("mm修正方差"))<<QString::number(MM_std);
        writeTocsv(saveFilelPath,strList);
        strList.clear();
        strList<<(QString::number(p1)+QStringLiteral("mm_peak均值"))<<QString::number(Peak_mean);
        writeTocsv(saveFilelPath,strList);
        strList.clear();
        strList<<(QString::number(p1)+QStringLiteral("mm极差"))<<QString::number(jicha_mm);
        writeTocsv(saveFilelPath,strList);
        strList.clear();
        strList<<(QString::number(p1)+QStringLiteral("mm脉宽"))<<QString::number(MaiKuan_mean);
        writeTocsv(saveFilelPath,strList);
        strList.clear();
    ...
    }else if(p3 == distance)
    {
        ...
        QStringList strList;
        strList<<(QString::number(p3)+QStringLiteral("mm均值"))<<QString::number(LSB_mean);
        writeTocsv(saveFilelPath,strList);
        strList.clear();
        strList<<(QString::number(p3)+QStringLiteral("mm方差"))<<QString::number(LSB_std);
        writeTocsv(saveFilelPath,strList);
        strList.clear();
        strList<<(QString::number(p3)+QStringLiteral("mm修正均值"))<<QString::number(MM_mean);
        writeTocsv(saveFilelPath,strList);
        strList.clear();
        strList<<(QString::number(p3)+QStringLiteral("mm修正方差"))<<QString::number(MM_std);
        writeTocsv(saveFilelPath,strList);
        strList.clear();
        strList<<(QString::number(p3)+QStringLiteral("mm_peak均值"))<<QString::number(Peak_mean);
        writeTocsv(saveFilelPath,strList);
        strList.clear();
        strList<<(QString::number(p3)+QStringLiteral("mm极差"))<<QString::number(jicha_mm);
        writeTocsv(saveFilelPath,strList);
        strList.clear();
        strList<<(QString::number(p1)+QStringLiteral("mm脉宽"))<<QString::number(MaiKuan_mean);
        writeTocsv(saveFilelPath,strList);
